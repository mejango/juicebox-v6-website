...
COMPONENT_SPECS.allowance = {
  file: 'discover.js (buildUseAllowanceModal, readAllowanceAccess, quotedOutputFloor)',
...
var COMPONENT_SDK_STARTS = {
  ...
  payouts: [
    'There is no confirmed high-level `sendPayoutsOf` builder in the current SDK export surface. Use root-package `jbMultiTerminalAbi`/`jbContractAddress` plus `/v6` `getAccountingContexts`, `getCurrentRuleset`, and `tokenCurrencyId`, then preserve Juicescan\'s limit, price, permission, simulation, and minimum-output orchestration.',
  ],
  allowance: [
    'There is no confirmed high-level `useAllowanceOf` builder in the current SDK export surface. Use root-package `jbMultiTerminalAbi`/`jbContractAddress` plus `/v6` `getAccountingContexts`, `getCurrentRuleset`, `hasPermissions`, and `JBPermissionIdsV6`; use Juicescan\'s exact eight-argument builder and live limit/surplus checks.',
  ],
  mint: ['`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/tokens.ts`): `buildMintTokensTx`, `getTokenAddress`, and the permission helpers.'],
  burn: ['`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/tokens.ts`): `buildBurnTokensTx`, plus `getCreditBalance`/`getTokenAddress` for holder state.'],
  'deploy-erc20': ['`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/tokens.ts`): `buildDeployErc20Tx` and `getTokenAddress`.'],
  reserved: ['No confirmed high-level reserved-token distribution builder is exported. Use root-package `jbControllerAbi`/`jbContractAddress` for `sendReservedTokensToSplitsOf` and its pending-balance read, with the Juicescan component as the workflow reference.'],
  permissions: ['`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/permissions.ts`): `buildSetPermissionsTx`, `hasPermissions`, `JBPermissionIdsV6`, `JBPermissionCatalogV6`, and bitmap encode/decode helpers.'],
  launch: [
    '`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/launch.ts`): `buildLaunchProjectTx`, `buildRulesetConfiguration`, `buildRulesetMetadata`, `buildTerminalConfigurations`, `buildAccountingContext`, `getProjectCreationFee`, and launch-log project-id decoders.',
    '`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/omnichain.ts` and `revnets.ts`): `buildOmnichainLaunchProjectTx` or `buildDeployRevnetTx` when that is the selected product/path.',
  ],
  'queue-ruleset': ['`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/rulesets.ts` and `omnichain.ts`): `buildQueueRulesetsTx`, `buildOmnichainQueueRulesetsTx`, `getCurrentRuleset`, `getUpcomingRuleset`, and `RULESET_WEIGHT_INHERIT`.'],
  loan: ['`@bananapus/nana-sdk-core/v6/loans` and `/v6/loan-math` (`packages/core/src/v6/loans.ts`, `loanMath.ts`): `buildBorrowTx`, `buildRepayLoanTx`, `buildReallocateCollateralTx`, `getBorrowableAmount`, fee constants, and loan proceeds math.'],
  move: ['`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/suckers.ts`): `buildBridgePrepareTx`, `buildToRemoteTx`, `buildBridgeClaimTx`, `getV6SuckerPairs`, transport classification/value helpers, and movement/proof helpers.'],
  'items-for-sale': ['`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/nft.ts`): `getProject721Shop`, tier metadata/category helpers, `effectiveTierPrice`, and 721 metadata helpers; use root `jb721TiersHookAbi` for the exact `adjustTiers` write.'],
  'transfer-ownership': ['The root `@bananapus/nana-sdk-core` package exports the canonical `jbProjectsAbi` and `jbContractAddress`; the transfer itself is the standard ERC-721 `transferFrom` path, coordinated per chain as Juicescan shows.'],
  'transfer-operator': ['Use root-package `revOwnerAbi`/deployment exports and `/v6` `isRevnetOperator` (`packages/core/src/v6/revnets.ts`); keep Juicescan\'s per-chain `setOperatorOf` orchestration.'],
  'edit-project': ['The root SDK exports `getProjectMetadata`, IPFS URI/CID helpers, and canonical `jbControllerAbi`/addresses. Juicescan remains the reference for pinning, content validation, and the per-chain `setUriOf` write.'],
  'token-metadata': ['Use root-package `jbControllerAbi`, `jbContractAddress`, and `/v6` `getTokenAddress`; Juicescan shows the per-chain `setTokenMetadataOf` orchestration and undeployed-token branch.'],
  'project-payer': ['`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/projectPayers.ts`): `buildDeployProjectPayerTx`, `normalizeProjectPayerMetadata`, `projectPayerFromDeployLogs`, and `JB_PROJECT_PAYER_DEPLOYER`.'],
  'accounting-token': ['`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/launch.ts`, `terminals.ts`, `currency.ts`): `buildAccountingContext`, `getAccountingContexts`, and `tokenCurrencyId`; use root `jbMultiTerminalAbi` for `addAccountingContextsFor`.'],
  'split-groups': ['`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/splits.ts`): `buildSetSplitGroupsTx`, `buildSplit`, `payoutSplitGroupId`, `RESERVED_TOKEN_SPLIT_GROUP_ID`, and `fillSplitPercents`.'],
  'add-liquidity': ['`@bananapus/nana-sdk-core/v6/uniswap-v4` (`packages/core/src/v6/uniswapV4.ts`): pool-id/state/price/tick/range/liquidity/amount helpers, swap direction/quote/build helpers, `buildPermit2ApproveTx`, and manager detection. The SDK supplies primitives; trace Juicescan for PositionManager action encoding, Safe fallback, and old-pool removal orchestration.'],
  'price-history': ['`@bananapus/nana-sdk-core/v6/uniswap-v4`: `uniswapV4PriceFromSqrtPriceX96`, pool-id/slot helpers, and price conversion helpers; root package: `downsampleTimeSeries` and Bendystraw filter/endpoint helpers.'],
  'claim-tokens': ['`@bananapus/nana-sdk-core/v6` (`packages/core/src/v6/tokens.ts`): `buildClaimTokensTx`, `getCreditBalance`, and `getTokenAddress`.'],
};
...
export function componentReproPrompt(title, prefix, fileHint) {
  ...
  lines.push('- Repository: https://github.com/Bananapus/juice-sdk-v4 — package `@bananapus/nana-sdk-core`, with V6 feature source under `packages/core/src/v6`. Install a version compatible with this site and verify its current exports instead of copying an old signature from this prompt.');
  ...
  lines.push('- Prefer the SDK’s typed builders, route preparers, reads, ABIs, and deployment data when they cover the feature. Juicescan intentionally implements its transaction paths independently at the ABI level: use it to understand complete UI orchestration, edge cases, and as a cross-check—not as evidence that the SDK was unknown or should be bypassed.');
  lines.push('- Authority order: deployed contract ABI/NatSpec and source; then the current SDK source and typed builders for what they cover; then Juicescan’s live orchestration, pure builders, and encoding tests; then this summary. If they disagree, stop, identify the version/source mismatch, and reconcile it instead of guessing or silently reimplementing the SDK.');
